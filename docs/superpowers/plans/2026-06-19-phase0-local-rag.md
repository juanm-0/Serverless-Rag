...
def test_retrieval_measures_topk_not_citations():
    # Expected file was retrieved even though the answer never names it.
    score = score_question(
...
> Note: `score_question` scores retrieval against the **top-k retrieved paths**, not
> the LLM's citations (spec §6). The runner below calls `retrieve()` directly to get
> them, then `generate_answer()` for the answer — so retrieval quality is measured
> independently of citation behavior. See `eval/run_eval.py` for the authoritative code.
